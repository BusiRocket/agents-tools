...
export const main = async () => {
  ...
  const manifestPath =
    flagValue(process.argv, "--manifest") ?? join(process.cwd(), "machine", "skills.json")
  const libraryDir = flagValue(process.argv, "--library") ?? join(homedir(), ".agents")
  ...
  const snapshot = await createSkillSourceSnapshot(libraryDir, parsed.manifest, runId)
  for (const [index, plan] of plans.entries()) {
    const result = spawnSync("skillkit", plan.applyArgs, { encoding: "utf8", stdio: "pipe" })
    if (result.stdout.trim() !== "") console.log(result.stdout.trim())
    if (result.status !== 0) {
      if (result.stderr.trim() !== "") console.error(result.stderr.trim())
      console.error(`${plan.id}: installation failed; snapshot retained at ${snapshot.snapshotDir}`)
      process.exitCode = 1
      return
    }
    const source = parsed.manifest.sources.at(index)
    if (source === undefined) throw new Error(`${plan.id}: source plan index is inconsistent`)
    await importInstalledSkillSource(source, join(homedir(), ".claude", "skills"), libraryDir)
  }
  console.log(
    JSON.stringify({ mode: "applied", snapshotDir: snapshot.snapshotDir, plans }, null, 2),
  )
}
