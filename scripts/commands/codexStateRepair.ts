...
export const main = async () => {
  ...
  const corruptDerived = report.databases.some(
    ({ path, status }) => path.endsWith("memories_1.sqlite") && status === "corrupt",
  )
  const plan = {
  ...
  if (activity.active) {
    console.log(
      JSON.stringify({ ok: false, applied: false, blocked: activity.reasons, plan }, null, 2),
    )
    process.exitCode = 1
    return
  }
  ...
  const database = await rebuildDerivedDatabase({ codexDir, snapshotDir: snapshot.snapshotDir })
