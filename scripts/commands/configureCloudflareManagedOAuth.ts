...
export const main = async () => {
  ...
  if (!/^[a-f0-9]{32}$/u.test(accountId) || !/^[a-f0-9-]{36}$/u.test(applicationId)) {
    throw new Error("Cloudflare account or application identifier is invalid")
  }
  if (!/^[a-zA-Z0-9._-]+$/u.test(profile)) throw new Error("Alchemy profile name is invalid")

  const credentialPath = join(homedir(), ".alchemy", "credentials", profile, "cf-oauth.json")
  ...
  if (typeof credentials.access !== "string" || credentials.access.length === 0) {
    throw new Error(`Alchemy profile '${profile}' has no Cloudflare OAuth access token`)
  }
  ...
  if (!currentResponse.ok || currentBody.success !== true || currentBody.result === undefined) {
    throw new Error(
      ...
      JSON.stringify(
        {
          ...
          desired: {
            ...
            dynamicClientRegistration: desiredOAuth.dynamic_client_registration?.enabled === true,
            allowAnyOnLocalhost:
              desiredOAuth.dynamic_client_registration?.allow_any_on_localhost === true,
            allowAnyOnLoopback:
              desiredOAuth.dynamic_client_registration?.allow_any_on_loopback === true,
          },
        },
        null,
        2,
  ...
  if (!verifyResponse.ok || verifyBody.success !== true || verifyBody.result === undefined) {
    throw new Error(
  ...
  const verification = {
    ...
    dynamicClientRegistration: verifiedOAuth?.dynamic_client_registration?.enabled === true,
    allowAnyOnLocalhost:
      verifiedOAuth?.dynamic_client_registration?.allow_any_on_localhost === true,
    allowAnyOnLoopback: verifiedOAuth?.dynamic_client_registration?.allow_any_on_loopback === true,
  }
