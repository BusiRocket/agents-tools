...
export const main = async () => {
  ...
  const selection = parseConversationSources(flagValues(process.argv, "--source"))
  ...
  if ((await conversationPathExists(output)) && !process.argv.includes("--force")) {
  ...
  if (!report.ok) {
    console.error(JSON.stringify({ ok: false, errors: report.skipped }, null, 2))
    process.exitCode = 1
    return
  }

  if (manifest === undefined) throw new Error("conversation export manifest was not created")
  console.log(
    JSON.stringify(
      {
        ok: true,
