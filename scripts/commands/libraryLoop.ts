...
export const main = async () => {
  ...
  if (dryRun) {
    results.push({
      name: "Classify",
      ok: true,
      output: "skipped: dry runs never spend model quota. Run without --dry-run to classify.",
    })
    ...
    results.push(
      await runBinStage("Classify", bin("run-library-classify.ts"), ["--command", classifyCommand]),
    )
  }

  results.push(await runBinStage("Triggers", bin("run-library-triggers.ts"), passthrough))
  results.push(await runBinStage("Router audit", bin("run-library-router-audit.ts"), passthrough))
  results.push(
  ...
  results.push(await runBinStage("Auto-park", bin("run-library-auto.ts"), passthrough))
  results.push(await runBinStage("Descriptions", bin("run-library-describe.ts"), []))
  ...
  console.log(`stages: ${String(results.length)}, failed: ${String(failed.length)}`)
