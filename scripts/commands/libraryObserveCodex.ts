...
export const main = async () => {
  ...
  for (const file of files) {
    for (const [skill, count] of Object.entries(await readCodexSkillReadsFromFile(file))) {
  ...
  const ranked = Object.entries(counts).toSorted((left, right) => right[1] - left[1])
  ...
  console.log(
    asJson
      ...
      : [
          ...
          ...ranked
            .slice(0, 15)
            .map(([skill, count]) => `  ${String(count).padStart(4)}  ${skill}`),
        ].join("\n"),
  )
}
