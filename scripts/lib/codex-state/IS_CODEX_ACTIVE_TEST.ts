...
void test("an exact Codex process blocks mutation", async () => {
  ...
  const activity = await isCodexActive(codexDir, "123 /usr/local/bin/codex app-server\n")
...
void test("similarly named tools do not create a false process match", async () => {
  ...
  const activity = await isCodexActive(codexDir, "123 /Applications/CodexBar.app/CodexBar\n")
