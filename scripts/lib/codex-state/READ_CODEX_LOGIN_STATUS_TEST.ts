...
void test("Codex login output is reduced to a non-secret authentication mode", async () => {
  const chatgpt = await createProbeExecutable("printf 'Logged in using ChatGPT\\n'")
  const api = await createProbeExecutable("printf 'Logged in using an API key\\n'")
  const signedOut = await createProbeExecutable("printf 'Not logged in\\n'; exit 1")
