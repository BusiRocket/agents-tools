...
void test("archive planning respects the 90-day boundary, current month, and malformed dates", async () => {
  ...
  const exactlyNinetyDays = "2026/05/20/rollout-2026-05-20T12-00-00-exactly-ninety-days.jsonl"
  const olderThanNinetyDays = "2026/05/20/rollout-2026-05-20T11-59-59-older-than-ninety-days.jsonl"
  const currentMonth = "2026/08/01/rollout-2026-08-01T00-00-00-current-month.jsonl"
...
void test("archive moves are checksummed and restore refuses a destination collision", async () => {
  ...
  const applied = await applySessionArchive(plan, archiveRoot, "archive-run", "")
