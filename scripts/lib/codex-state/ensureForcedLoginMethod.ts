...
export const ensureForcedLoginMethod = (contents: string): ForcedLoginMethodResult => {
  const lines = contents.split("\n")
  const assignmentPattern = /^(\s*forced_login_method\s*=\s*)"([^"]*)"(\s*(?:#.*)?)$/
  const assignmentIndexes = lines.flatMap((line, index) =>
    assignmentPattern.test(line) ? [index] : [],
  )
  if (assignmentIndexes.length > 1) {
    return { ok: false, errors: ["forced_login_method must appear at most once"] }
  }
  ...
  if (assignmentIndex !== undefined) {
    ...
    if (hasTableBefore) {
      return { ok: false, errors: ["forced_login_method must be a top-level setting"] }
    }
    ...
    if (match === null) return { ok: false, errors: ["forced_login_method is malformed"] }
  ...
  let insertionIndex = lines.findIndex((line) => line.trimStart().startsWith("["))
