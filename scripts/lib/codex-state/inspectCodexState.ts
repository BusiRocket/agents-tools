...
export const inspectCodexState = async (codexDir: string): Promise<CodexStateReport> => {
  const databases = await Promise.all(
    CODEX_DATABASES.map((name) => runSqliteIntegrityCheck(join(codexDir, name))),
  )
  const rolloutPaths = (await listCodexRollouts(join(codexDir, "sessions"))).toSorted(
  ...
  const findings = await Promise.all(
    rolloutPaths.map(async (path) => {
      try {
        ...
        return { path, status: "malformed" as const, summary: "session is unreadable" }
      }
    }),
  )
  ...
  return {
    ...
    sessionBytes: sessionStats.reduce((total, current) => total + current.size, 0),
