...
export const readRolloutHeader = async (path: string): Promise<SessionFinding> => {
  ...
  try {
    ...
    if (firstRecord === undefined) {
      return { path, status: "malformed", summary: "session has no usable records" }
    }
    ...
    try {
      ...
      return { path, status: "malformed", summary: "first record is not valid JSON" }
    }
    if (typeof parsed !== "object" || parsed === null || !("type" in parsed)) {
      return { path, status: "malformed", summary: "first record is not session_meta" }
    }
    ...
    if (typeof payload !== "object" || payload === null || !("id" in payload)) {
      return { path, status: "malformed", summary: "session_meta has no session id" }
    }
