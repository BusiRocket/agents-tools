...
export const rebuildDerivedDatabase = async (
  ...
  const integrity = await runSqliteIntegrityCheck(join(options.codexDir, databaseName))
  if (integrity.status !== "corrupt") return { status: "not-corrupt", entries: [], reasons: [] }

  const verification = await verifyCodexSnapshot(options.snapshotDir)
  if (!verification.ok) {
    return { status: "snapshot-invalid", entries: [], reasons: verification.errors }
  }
  const manifest = JSON.parse(
    await readFile(join(options.snapshotDir, "manifest.json"), "utf8"),
  ) as { entries?: { relativePath?: string }[] }
  if (manifest.entries?.some(({ relativePath }) => relativePath === databaseName) !== true) {
    return {
