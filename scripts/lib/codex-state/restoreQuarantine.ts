...
export const restoreQuarantine = async (
  ...
  if (activity.active) return { status: "blocked", entries: [], reasons: activity.reasons }
  const manifest = await readValidQuarantineManifest(options.snapshotDir)
  if (manifest === undefined) {
    return { status: "invalid", entries: [], reasons: ["quarantine manifest is invalid"] }
  }
  ...
  for (const entry of manifest.entries) {
    ...
    if (await pathExists(destinationPath)) {
      collisions.push(`restore destination exists: ${entry.originalRelativePath}`)
    }
    try {
      const sourceStat = await stat(sourcePath)
      if (sourceStat.size !== entry.bytes || (await hashFile(sourcePath)) !== entry.sha256) {
        verificationErrors.push(`quarantine verification failed: ${entry.destinationRelativePath}`)
      }
    } catch {
      verificationErrors.push(`quarantine file is missing: ${entry.destinationRelativePath}`)
    }
  }
  if (collisions.length > 0) return { status: "collision", entries: [], reasons: collisions }
  if (verificationErrors.length > 0) {
    return { status: "invalid", entries: [], reasons: verificationErrors }
  }
  if (options.dryRun) return { status: "planned", entries: manifest.entries, reasons: [] }
  ...
  for (const entry of manifest.entries) {
    ...
    if ((await hashFile(destinationPath)) !== entry.sha256) {
      throw new Error(`restored quarantine hash mismatch: ${entry.originalRelativePath}`)
    }
