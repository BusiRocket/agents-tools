...
export const restoreSessionArchive = async (
  ...
  const activity = await isCodexActive(dirname(options.sessionsDir), options.processTable)
  if (activity.active) {
    return { status: "blocked", runDir: options.runDir, entries: [], reasons: activity.reasons }
  }
  ...
  if (collisions.length > 0) {
    return { status: "collision", runDir: options.runDir, entries: [], reasons: collisions }
  }
  const verificationErrors = await verifySessionArchive(options.runDir, manifest)
  ...
  if (options.dryRun) {
    return { status: "planned", runDir: options.runDir, entries: manifest.entries, reasons: [] }
  }
  ...
  return { status: "restored", runDir: options.runDir, entries: manifest.entries, reasons: [] }
}
