...
export const runSqliteIntegrityCheck = async (path: string): Promise<DatabaseIntegrity> => {
  try {
  ...
  return new Promise((resolve) => {
    ...
    child.on("error", () => {
      resolve({ path, status: "unreadable", summary: "sqlite3 could not inspect database" })
    })
