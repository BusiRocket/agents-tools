...
export const verifyCodexSnapshot = async (snapshotDir: string): Promise<SnapshotVerification> => {
  let parsed: unknown
  try {
    parsed = JSON.parse(await readFile(join(snapshotDir, "manifest.json"), "utf8")) as unknown
  } catch {
