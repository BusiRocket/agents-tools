...
export const verifySnapshotEntry = async (
  ...
  if (typeof rawEntry !== "object" || rawEntry === null) return ["manifest entry is invalid"]
  ...
  try {
    ...
    if (fileStat.size !== entry.bytes) errors.push(`${entry.relativePath}: byte count mismatch`)
    if ((await hashFile(path)) !== entry.sha256) errors.push(`${entry.relativePath}: hash mismatch`)
    return errors
  } catch {
