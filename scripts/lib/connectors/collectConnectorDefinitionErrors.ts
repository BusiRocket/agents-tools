export const collectConnectorDefinitionErrors = (
  ...
  if (
    ...
    connector.profiles.some((profile) => typeof profile !== "string" || !profiles.has(profile))
  ) {
    errors.push(`${path}.profiles contains an unknown or missing profile`)
  }
  if (typeof connector.ownership !== "string" || !ownershipKinds.has(connector.ownership)) {
  ...
  if (typeof connector.criticality !== "string" || !criticalities.has(connector.criticality)) {
    errors.push(`${path}.criticality is invalid`)
  }
  if (
    typeof connector.endpoint === "string" &&
    /[?&](?:token|api[_-]?key|access[_-]?token|authorization)=/i.test(connector.endpoint)
  ) {
