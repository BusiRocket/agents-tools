  ...
  await fs.writeFile(
    ...
    [
      ...
      JSON.stringify({
        ...
        message: { role: "assistant", content: ["password", "supersecretvalue"].join("=") },
      }),
    ].join("\n"),
  )
  await fs.writeFile(
    ...
    [
      ...
      JSON.stringify({
        ...
        payload: { type: "function_call", name: "exec_command", arguments: "pnpm check" },
      }),
    ].join("\n"),
  )

  const report = await captureConversations(home, new Set(["claude-code", "codex"]))

  assert.equal(report.ok, true)
  assert.equal(report.records.length, 2)
  const claude = report.records.find((record) => record.source === "claude-code")
  const codex = report.records.find((record) => record.source === "codex")
  if (claude === undefined || codex === undefined) throw new Error("expected both source records")
...
void test("capture ignores credential stores, caches, and SQLite sidecars", async (context) => {
  const home = await fs.mkdtemp(join(tmpdir(), "rocket-conversations-exclusions-"))
  context.after(async () => fs.rm(home, { recursive: true, force: true }))
  const root = join(home, ".claude", "projects", "project-a")
  await fs.mkdir(join(root, "tool-results"), { recursive: true })
  await fs.writeFile(join(root, "tool-results", "secret.json"), '{"message":"do not ingest"}')
...
void test("capture fails closed on malformed conversation JSON", async (context) => {
  const home = await fs.mkdtemp(join(tmpdir(), "rocket-conversations-malformed-"))
  context.after(async () => fs.rm(home, { recursive: true, force: true }))
