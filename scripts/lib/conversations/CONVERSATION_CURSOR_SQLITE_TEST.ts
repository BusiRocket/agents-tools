...
void test("Cursor SQLite capture groups current and legacy conversation storage", async () => {
  ...
  const insertCurrent = database.prepare("INSERT INTO cursorDiskKV(key, value) VALUES (?, ?)")
  insertCurrent.run(
    "composerData:session-1",
    JSON.stringify({ name: "Current session", conversation: [{ type: 1, text: "inline" }] }),
  )
  insertCurrent.run(
    "bubbleId:session-1:bubble-1",
    JSON.stringify({ type: "assistant", text: "separate", toolResults: ["done"] }),
  )
  database
