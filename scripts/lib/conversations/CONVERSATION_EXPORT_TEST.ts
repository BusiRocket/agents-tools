...
void test("exports carry a verified manifest and reject tampering", async (context) => {
  ...
  assert.equal(tampered.errors.includes("export content hash does not match the manifest"), true)
})
...
void test("streamed exports preserve manifests without retaining the archive in memory", async (context) => {
  ...
  try {
    ...
    await writeConversationExportFromStore(store, output, new Date("2026-08-19T10:00:00Z"))
  } finally {
    store.close()
  }
...
void test("imports are dry-run by default and back up an existing archive on apply", async (context) => {
  ...
  await writeConversationExport([record], input, new Date("2026-08-19T10:00:00Z"))

  const dryRun = await importConversationExport({ input, archive, apply: false })
