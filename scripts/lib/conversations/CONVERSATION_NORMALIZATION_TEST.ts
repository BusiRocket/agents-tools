...
void test("conversation fragments with the same source session retain every event", () => {
  const first = conversationRecordFromDocument({
    contents: JSON.stringify({ sessionID: "session-1", role: "user", text: "first" }),
    relativePath: "part/first.json",
    source: "opencode",
    sourceIdHint: "first",
  })
  const second = conversationRecordFromDocument({
    contents: JSON.stringify({ sessionID: "session-1", role: "assistant", text: "second" }),
  ...
  assert.ok(second)
  assert.deepEqual(
    mergeConversationRecordFragments(first, second).events.map(({ text }) => text),
    ["first", "second"],
  )
})
