...
void test("OpenCode SQLite capture joins sessions, messages, and parts", async () => {
  ...
  database
    ...
    .run("session-1", "OpenCode session", "/workspace", null, 1_700_000_000_000, 1_700_000_001_000)
  database
    .prepare("INSERT INTO message VALUES (?, ?, ?, ?)")
    .run("message-1", "session-1", 1_700_000_000_000, JSON.stringify({ role: "user" }))
  database
    .prepare("INSERT INTO part VALUES (?, ?, ?, ?)")
    .run("part-1", "message-1", 1_700_000_000_000, JSON.stringify({ type: "text", text: "hello" }))
  database.close()
...
void test("OpenCode desktop capture decodes bounded Tauri stores", async () => {
  ...
  const value = Buffer.from(JSON.stringify({ messages: [{ role: "user", content: "desktop" }] }))
