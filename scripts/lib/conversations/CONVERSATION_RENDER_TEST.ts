...
void test("render produces a dry-run plan and a MemPalace-readable Markdown corpus", async (context) => {
  ...
  const dryRun = await renderConversationArchive({ input, outputDirectory, apply: false })
  assert.equal(dryRun.applied, false)
  await assert.rejects(fs.access(outputDirectory))

  const applied = await renderConversationArchive({ input, outputDirectory, apply: true })
  assert.equal(applied.files, 1)
  const rendered = await fs.readFile(join(outputDirectory, `${record.id}.md`), "utf8")
