...
void test("the source catalog covers the union of both reference projects", () => {
  assert.deepEqual(
    ...
    [...CONVERSATION_SOURCES].toSorted((left, right) => left.localeCompare(right)),
  )
...
void test("SQLite extraction is restricted to known conversation-bearing tables", () => {
  assert.match(sqliteConversationQuery("ItemTable", "trae") ?? "", /conversation/u)
  assert.match(sqliteConversationQuery("part", "opencode") ?? "", /SELECT rowid/u)
...
void test("OpenCode desktop discovery accepts only Tauri store files", () => {
  ...
  assert.equal(isSupportedSourceArtifact(definition, ".local/share/ai.opencode.app", "tauri"), true)
  assert.equal(isSupportedSourceArtifact(definition, ".local/share/ai.opencode.app", "json"), false)
})
