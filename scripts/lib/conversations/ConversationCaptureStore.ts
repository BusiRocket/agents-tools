...
export class ConversationCaptureStore {
  ...
  constructor(path: string) {
    ...
    this.#find = this.#database.prepare("SELECT record_json FROM records WHERE id = ?")
  ...
  mergeFragment(record: ConversationRecord) {
    ...
    this.#upsert.run(merged.id, JSON.stringify(merged), merged.provenance.redactions)
  }

  replace(record: ConversationRecord): ConversationStoreChange {
    const existing = this.#find.get(record.id)
    if (existing === undefined || typeof existing.record_json !== "string") {
      this.#upsert.run(record.id, JSON.stringify(record), record.provenance.redactions)
      return "added"
    }
    const current = JSON.parse(existing.record_json) as ConversationRecord
    if (current.provenance.contentSha256 === record.provenance.contentSha256) return "duplicate"
    this.#upsert.run(record.id, JSON.stringify(record), record.provenance.redactions)
    return "updated"
  }

  *serializedRecords() {
    const statement = this.#database.prepare("SELECT record_json FROM records ORDER BY id")
  ...
  count() {
    const row = this.#database.prepare("SELECT count(*) AS total FROM records").get()
