...
export const captureConversationArtifact = async (
  ...
  try {
    const records = (await readConversationArtifact(artifact)).flatMap((document) => {
      const record = conversationRecordFromDocument(document)
      return record === undefined ? [] : [redactConversationHome(record, home)]
    })
    return { source: artifact.source, relativePath: artifact.relativePath, records }
  } catch (error) {
