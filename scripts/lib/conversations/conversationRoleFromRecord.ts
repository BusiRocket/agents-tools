...
export const conversationRoleFromRecord = (record: unknown): ConversationRole => {
  ...
  if (candidate !== undefined && CONVERSATION_ROLES.has(candidate as ConversationRole)) {
    return candidate as ConversationRole
  }
  if (candidate?.includes("user") === true || candidate === "human") return "user"
  if (candidate?.includes("assistant") === true || candidate === "ai") return "assistant"
  if (candidate?.includes("tool") === true || candidate?.includes("function") === true)
    return "tool"
