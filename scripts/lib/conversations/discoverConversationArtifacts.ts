...
export const discoverConversationArtifacts = async (
  ...
  for (const definition of sourceDefinitions) {
    if (selectedSources !== undefined && !selectedSources.has(definition.id)) continue

    artifacts.push(...(await discoverSourceDefinitionArtifacts(home, definition)))
  }

  const unique = new Map<string, ConversationArtifact>()
  for (const artifact of artifacts) unique.set(`${artifact.source}:${artifact.path}`, artifact)
