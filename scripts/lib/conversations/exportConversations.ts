...
export const exportConversations = async (
  ...
  const directory = await mkdtemp(join(tmpdir(), "rocket-agents-conversation-export-"))
  const store = new ConversationCaptureStore(join(directory, "capture.sqlite"))
  try {
    const report = await captureConversationArtifacts(home, selectedSources, (record) => {
