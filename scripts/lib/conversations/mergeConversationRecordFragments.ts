...
export const mergeConversationRecordFragments = (
  ...
  if (left.id !== right.id || left.source !== right.source || left.sourceId !== right.sourceId) {
  ...
  const hashes = [left.provenance.contentSha256, right.provenance.contentSha256].toSorted((a, b) =>
    a.localeCompare(b),
  )
  const relativePaths = [left.provenance.relativePath, right.provenance.relativePath].toSorted(
    (a, b) => a.localeCompare(b),
  )
  const timestamps = [...events.values()]
    .flatMap((event) => (event.timestamp === undefined ? [] : [event.timestamp]))
