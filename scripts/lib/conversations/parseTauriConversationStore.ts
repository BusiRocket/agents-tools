...
export const parseTauriConversationStore = (data: Buffer) => {
  ...
  while (offset < data.length) {
    if (offset + 4 > data.length) throw new Error("truncated Tauri store key length")
    ...
    if (offset + 4 > data.length) throw new Error("truncated Tauri store value length")
    const valueBytes = data.readUInt32LE(offset)
    offset += 4
    if (offset + valueBytes > data.length) throw new Error("invalid Tauri store value length")
