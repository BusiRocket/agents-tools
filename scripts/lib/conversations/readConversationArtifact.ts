...
export const readConversationArtifact = async (artifact: ConversationArtifact) => {
  if (artifact.storage === "sqlite") return readSqliteConversationDocuments(artifact)
  if (artifact.storage === "tauri") return readTauriConversationDocuments(artifact)
