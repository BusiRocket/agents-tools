...
export const readCursorItemTableConversationDocuments = (artifact: ConversationArtifact) => {
  ...
  try {
    ...
    for (const row of statement.iterate()) {
      ...
      if (Buffer.byteLength(row.value) > MAX_CONVERSATION_FILE_BYTES) {
        throw new Error(`Cursor conversation record exceeds the safe size limit: ${row.key}`)
      }
      const recordId = typeof row.record_id === "number" ? String(row.record_id) : row.key
      documents.push({
        contents: row.value,
