...
export const readSqliteConversationDocuments = (
  ...
  const specialized = readSpecializedSqliteConversationDocuments(artifact, tables)
  if (specialized !== undefined) return specialized

  for (const table of tables) {
    if (artifact.source === "cursor" && ["cursorDiskKV", "ItemTable"].includes(table)) continue
    ...
    for (const row of rows) {
      ...
      documents.push({
        contents: typeof row.value === "string" ? row.value : JSON.stringify(row),
