...
export const readTauriConversationDocuments = async (artifact: ConversationArtifact) => {
  const handle = await fs.open(artifact.path, constants.O_RDONLY | constants.O_NOFOLLOW)
  try {
    const stat = await handle.stat()
    if (!stat.isFile()) throw new Error(`not a regular file: ${artifact.relativePath}`)
    ...
    return parseTauriConversationStore(await handle.readFile())
      ...
      .flatMap(({ key, value }) => {
        ...
        if (!Array.isArray(object.messages) && !Array.isArray(object.history)) return []
        return [
          {
