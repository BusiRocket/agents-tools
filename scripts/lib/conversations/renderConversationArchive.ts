...
export const renderConversationArchive = async (options: {
  ...
  const directory = await mkdtemp(join(tmpdir(), "rocket-agents-conversation-render-"))
  ...
  try {
    ...
    if (options.apply) {
      ...
      for (const serialized of store.serializedRecords()) {
        ...
        await fs.writeFile(temporary, renderConversationRecordMarkdown(record), { mode: 0o600 })
