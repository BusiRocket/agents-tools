...
export const renderConversationRecordMarkdown = (record: ConversationRecord) =>
  [
    ...
    ...(record.startedAt === undefined ? [] : [`- Started: ${record.startedAt}`]),
    ...(record.updatedAt === undefined ? [] : [`- Updated: ${record.updatedAt}`]),
    `- Content SHA-256: ${record.provenance.contentSha256}`,
    "",
    ...record.events.flatMap((event) => [conversationEventHeading(event), "", event.text, ""]),
  ].join("\n")
