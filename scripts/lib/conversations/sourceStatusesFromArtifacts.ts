...
export const sourceStatusesFromArtifacts = (artifacts: ConversationArtifact[]) => {
  const statuses: ConversationSourceStatus[] = []

  for (const definition of sourceDefinitions) {
    const matches = artifacts.filter((artifact) => artifact.source === definition.id)
    ...
    statuses.push({
      ...
      databases: matches.filter((artifact) => artifact.storage === "sqlite").length,
      skipped: 0,
      ...(available
        ? {}
        : {
