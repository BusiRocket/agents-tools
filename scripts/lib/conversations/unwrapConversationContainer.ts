...
export const unwrapConversationContainer = (value: unknown, depth = 0): unknown[] => {
  if (depth > 4) return []
  if (Array.isArray(value)) return value
  if (typeof value === "string") {
    const nested = parseJsonValue(value)
    return nested === undefined ? [value] : unwrapConversationContainer(nested, depth + 1)
  }
  if (typeof value !== "object" || value === null) return [value]

  const object = value as Record<string, unknown>
  if (Array.isArray(object.tabs)) {
    return object.tabs.flatMap((tab) => unwrapConversationContainer(tab, depth + 1))
  }
  ...
  for (const key of ["value", "data"] as const) {
    ...
    if (nested !== undefined) return unwrapConversationContainer(nested, depth + 1)
  }
  return [value]
}
