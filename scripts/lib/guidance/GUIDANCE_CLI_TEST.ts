...
void test("guidance commands accept the public config and run spellings", () => {
  ...
  assert.equal(
    readAliasedGuidanceFlag(["node", "rollback", "--run", "run-1"], "--run", "--to"),
    "run-1",
  )
})
...
void test("guidance compatibility aliases remain accepted", () => {
  assert.equal(
    readAliasedGuidanceFlag(["--canonical-dir", "/canonical"], "--config", "--canonical-dir"),
    "/canonical",
  )
  assert.equal(readAliasedGuidanceFlag(["--to", "run-1"], "--run", "--to"), "run-1")
})

void test("guidance flags reject missing and conflicting values", () => {
  assert.throws(() => readGuidanceFlag(["--config"], "--config"), /missing value/u)
  assert.throws(
  ...
  assert.throws(
    () => readAliasedGuidanceFlag(["--run", "run-1", "--to", "run-2"], "--run", "--to"),
    /conflicting values/u,
  )
})
