...
void test("guidance fast paths are deterministic and conflict-aware", () => {
  ...
  const createSources = (published = false, auxiliary = "rules"): GuidanceSources => {
    const old = {
      ...
      "live/claude/CLAUDE.md": "Never expose credentials.\n\nOld Claude overlay.\n",
      "live/codex/AGENTS.md": "Never expose credentials.\n\nOld Codex overlay.\n",
    ...
    return {
      ...
      hashes: Object.fromEntries(
        Object.entries(values).map(([path, value]) => [path, sha256Text(value)]),
      ),
    }
  }
  assert.equal(
    resolveGuidanceFastPath({ sources: createSources(), policy, acceptPublished: false }).kind,
    "converged",
  )
  ...
  assert.equal(
    resolveGuidanceFastPath({ sources: createSources(true), policy, acceptPublished: false }).kind,
    "reconcile",
  )
  assert.equal(
