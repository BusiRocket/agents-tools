...
void test("input hashes use a strict normalized array and match every source exactly", () => {
  ...
  const cases = [
    ...
    {
      name: "stale",
      inputHashes: [{ path: "canonical/shared.md", sha256: "d".repeat(64) }, result.inputHashes[1]],
      expectedError: /do not match/u,
    },
  ]
  for (const fixture of cases) {
    ...
    assert.equal(validated.ok, false, `${fixture.name} input hashes must be rejected`)
