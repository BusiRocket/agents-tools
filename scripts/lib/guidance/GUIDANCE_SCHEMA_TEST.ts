...
void test("the reconciliation output schema is strict at every object boundary", () => {
  const visit = (value: unknown, path: string): void => {
    ...
    if (schemaNode.type === "object") {
      ...
      assert.notEqual(schemaNode.properties, null, `${path} must declare properties`)
      assert.deepEqual(
        [...((schemaNode.required as string[] | undefined) ?? [])].sort((left, right) =>
          left.localeCompare(right),
        ),
        Object.keys(schemaNode.properties as Record<string, unknown>).sort((left, right) =>
          left.localeCompare(right),
        ),
        `${path} must require every declared property`,
      )
    }
    for (const [key, child] of Object.entries(schemaNode)) visit(child, `${path}.${key}`)
  }
