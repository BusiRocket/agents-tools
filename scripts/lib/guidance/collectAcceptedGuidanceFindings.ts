...
export const collectAcceptedGuidanceFindings = async (
  ...
  try {
    accepted = parseAcceptedGuidanceState(
      JSON.parse(await readFile(join(stateDir, "accepted.json"), "utf8")) as unknown,
    )
  } catch {
  ...
  for (const name of Object.keys(contents) as (keyof GuidanceDocumentContents)[]) {
    const content = contents[name]
    if (content !== undefined && sha256Text(content) !== accepted.state.outputHashes[name])
