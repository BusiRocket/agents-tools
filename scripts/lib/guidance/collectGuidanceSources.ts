...
export const collectGuidanceSources = async (options: {
  ...
  await Promise.all([
    ...
    add("canonical/claude-overlay.md", join(options.canonicalDir, "claude-overlay.md")),
    add("canonical/codex-overlay.md", join(options.canonicalDir, "codex-overlay.md")),
    ...
    add(
      ...
      options.rulesInventoryPath ?? join(process.cwd(), "dist", "markdown", "CLAUDE.md"),
    ),
  ])
