...
export const parseAcceptedGuidanceState = (
  ...
  if (!isRecord(raw)) return { ok: false, error: "accepted guidance state must be an object" }
  if (
  ...
  const outputKeys = ["shared", "claudeOverlay", "codexOverlay", "claudeDocument", "codexDocument"]
  if (
    Object.keys(outputHashes).length !== outputKeys.length ||
    !outputKeys.every((key) => Object.hasOwn(outputHashes, key)) ||
    !outputKeys.every(
      (key) => typeof outputHashes[key] === "string" && /^[a-f0-9]{64}$/u.test(outputHashes[key]),
    )
  )
  ...
  return {
    ...
    state: {
      ...
      outputHashes: outputHashes as unknown as AcceptedGuidanceState["outputHashes"],
    },
  }
}
