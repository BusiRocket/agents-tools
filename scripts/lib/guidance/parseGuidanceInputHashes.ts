...
export const parseGuidanceInputHashes = (
  raw: unknown,
): { ok: true; inputHashes: GuidanceInputHash[] } | { ok: false; error: string } => {
  if (!Array.isArray(raw) || raw.length === 0)
    return { ok: false, error: "inputHashes must be a non-empty path and sha256 array" }
  ...
  return {
    ok: true,
    inputHashes: inputHashes.toSorted((left, right) => left.path.localeCompare(right.path)),
  }
}
