...
export const reclaimGuidanceLock = async (
  ...
  try {
    try {
      await writeFile(recoveryPath, owner, { encoding: "utf8", flag: "wx", mode: 0o600 })
      ...
      const recovery = await readGuidanceLockObservation(recoveryPath, staleAfterMs)
      if (recovery?.active === true) return undefined
      if (recovery !== undefined) await createGuidanceLockRelease(recoveryPath, recovery.owner)()
      return undefined
    }
    const current = await readGuidanceLockObservation(lockPath, staleAfterMs)
    if (current === undefined || current.active || !isSameGuidanceLock(observed, current))
      return undefined
