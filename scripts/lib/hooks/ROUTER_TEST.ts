...
void test("every real prompt reaches its lane", () => {
  ...
  for (const [lane, prompts] of Object.entries(ROUTER_FIXTURES.routes)) {
    ...
    for (const prompt of prompts) {
      ...
      assert.notEqual(directive, null, `expected ${lane}, got silence for: ${prompt}`)
      assert.match(String(directive), new RegExp(marker), `wrong lane for: ${prompt}`)
    }
  }
})
...
void test("machine-generated payloads never route", () => {
  assert.equal(runRouterHook("<task-notification>done</task-notification>"), null)
})
...
void test("malformed stdin does not crash the hook", () => {
  ...
  assert.equal(execFileSync("bash", [hook], { input: "not json", encoding: "utf8" }).trim(), "")
})
