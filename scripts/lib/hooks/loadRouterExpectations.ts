...
export const loadRouterExpectations = async (path: string): Promise<RouterExpectationsManifest> => {
  ...
  if (record.version !== 1 || !Array.isArray(record.expectations)) {
    throw new Error("router expectations need version 1 and an expectations array")
  }
