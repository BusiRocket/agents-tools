...
void test("an unknown skill is refused by name", () => {
  const result = applyTransition(FAN_OUT_MANIFEST, "nope", "adopted", {}, "2026-08-18")
  assert.ok(!result.ok)
  assert.match(result.error, /not in the library/)
})

void test("a transition that would break the state's obligations is refused", () => {
  const result = applyTransition(FAN_OUT_MANIFEST, "adoptedEverywhere", "forked", {}, "2026-08-18")
  assert.ok(!result.ok)
...
void test("the original manifest is left untouched", () => {
  applyTransition(FAN_OUT_MANIFEST, "parkedOne", "adopted", { reason: "x" }, "2026-08-18")
