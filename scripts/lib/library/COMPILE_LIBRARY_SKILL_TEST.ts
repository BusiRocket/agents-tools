...
void test("Claude retains native fields while Codex receives a portable view", async () => {
  ...
  const claudeContents = await readFile(join(claude.outputPath, "SKILL.md"), "utf8")
  const codexContents = await readFile(join(codex.outputPath, "SKILL.md"), "utf8")
...
void test("a missing curated source is reported without aborting the target build", async () => {
  ...
  const result = await compileLibraryTarget(join(root, "skills"), join(root, "compiled"), "codex", [
    "core/missing",
  ])
