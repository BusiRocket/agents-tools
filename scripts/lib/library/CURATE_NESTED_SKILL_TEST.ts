...
void test("a nested skill can be adopted without adopting its whole bundle", () => {
  ...
  assert.ok(result.ok)
  assert.equal(result.manifest.entries["engineering-skills/google-workspace-cli"]?.state, "adopted")
...
void test("a nested skill under an unknown bundle is refused", () => {
  const result = applyTransition(NESTED_MANIFEST, "nope/whatever", "adopted", {}, "2026-08-18")
  assert.ok(!result.ok)
