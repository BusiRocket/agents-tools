...
export const listSkillPaths = async (libraryDir: string) => {
  ...
  try {
    ...
    bundles = entries.filter((entry) => entry.isDirectory()).map((entry) => entry.name)
  } catch {
    return paths
  }
  ...
  for (const bundle of bundles) {
    ...
    let children: string[]
    try {
      const entries = await fs.readdir(join(root, bundle), { withFileTypes: true })
      children = entries.filter((entry) => entry.isDirectory()).map((entry) => entry.name)
    } catch {
      continue
    }
