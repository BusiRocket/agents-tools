...
void test("a phrase that fires the lane owning the skill is correct", () => {
  assert.equal(classifyRouterOutcome("invoice-quarter-close", "invoice-ops"), "correct-lane")
})

void test("a phrase that fires nothing is the strongest gap evidence", () => {
  assert.equal(classifyRouterOutcome("invoice-quarter-close", undefined), "no-lane")
})
...
void test("a lane with no declared skills never counts as correct", () => {
  assert.equal(classifyRouterOutcome("anything", "environment-ops"), "wrong-lane")
})
