...
void test("a request naming what to do is a trigger", () => {
  assert.equal(isContinuationPhrase("arregla el header en movil, se ve mal"), false)
})

void test("a request about invoices is a trigger", () => {
  assert.equal(isContinuationPhrase("revisa todas las recurrentes de holded y su plantilla"), false)
})
