...
void test("the invoice directive identifies its lane", () => {
  assert.equal(
    laneFromContext("Invoice and tax work. Load the invoice-quarter-close skill"),
    "invoice-ops",
  )
})
...
void test("the debug directive identifies its lane", () => {
  assert.equal(
    laneFromContext("Use the superpowers:systematic-debugging skill before proposing a fix."),
    "debug",
  )
})
