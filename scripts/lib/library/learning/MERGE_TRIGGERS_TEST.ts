...
void test("measured phrases land on the matching entry", () => {
  ...
  assert.deepEqual(next.entries["brp-todo-work"]?.triggers, ["ejecuta el backlog"])
})
...
void test("a nested entry is matched by its skill name", () => {
  ...
  assert.deepEqual(next.entries["pm-skills/jira-expert"]?.triggers, ["crea un ticket"])
})
...
void test("existing triggers survive and duplicates are not added twice", () => {
  const seeded: CurationManifest = {
    version: 1,
    entries: { "brp-todo-work": { state: "adopted", triggers: ["ejecuta el backlog"] } },
  }
  const next = mergeTriggersIntoManifest(
    seeded,
  ...
  assert.deepEqual(next.entries["brp-todo-work"]?.triggers, ["ejecuta el backlog", "corre el todo"])
})
...
void test("the original manifest is left untouched", () => {
  mergeTriggersIntoManifest(TRIGGER_MANIFEST, { "brp-todo-work": ["x"] }, TRIGGER_KEYS, 10)
