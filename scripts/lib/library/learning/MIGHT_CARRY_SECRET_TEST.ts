...
void test("a pasted password-shaped string is caught", () => {
  assert.equal(mightCarrySecret("no me deja entrar, los datos son Kd%L6zwCMuo&p2As96s"), true)
})
...
void test("a token prefix is caught", () => {
  assert.equal(mightCarrySecret("usa ghp_0000000000000000000000000000000000"), true)
})
...
void test("a request mentioning a file path is not flagged", () => {
  assert.equal(mightCarrySecret("revisa /Users/someone/p/project/src/index.ts"), false)
})
