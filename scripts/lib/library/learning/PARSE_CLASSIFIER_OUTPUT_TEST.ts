...
void test("a clean classifier response parses", () => {
  const parsed = parseClassifierOutput('{"requests":[{"procedure":"read discord messages"}]}')
...
void test("the recurring_shape field name is accepted as well", () => {
  const parsed = parseClassifierOutput('{"requests":[{"recurring_shape":"y","project":"p"}]}')
...
void test("a nested object inside a string does not end the scan early", () => {
  assert.equal(firstJsonObject('{"a":"} not the end","b":1}'), '{"a":"} not the end","b":1}')
})
