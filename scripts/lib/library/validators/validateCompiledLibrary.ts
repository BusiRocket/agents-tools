...
export const validateCompiledLibrary = async (
  ...
  for (const skill of compiled) {
    ...
    if (finding.kind === "invalid") errors.push(`${skill.logicalName}: compiled skill is invalid`)
    ...
    if (target !== "claude" && claudeFields.length > 0) {
      errors.push(`${skill.logicalName}: compiled skill retains a Claude extension`)
    }
    if (target !== "claude" && finding.name !== skill.targetName) {
      errors.push(`${skill.logicalName}: compiled name does not match its target alias`)
    }
  }
  return errors
}
