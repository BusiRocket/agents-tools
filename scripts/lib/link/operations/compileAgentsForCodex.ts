...
export const compileAgentsForCodex = async ({
  ...
  for (const entry of entries) {
    ...
    const source = await fs.readFile(path.join(srcAgentsDir, entry.name), "utf8")
    const targetName = entry.name.replace(/\.md$/, ".toml")
    await fs.writeFile(path.join(outputAgentsDir, targetName), renderCodexAgent(source), "utf8")
