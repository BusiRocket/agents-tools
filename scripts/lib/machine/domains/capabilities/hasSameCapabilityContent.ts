...
export const hasSameCapabilityContent = async (
  ...
  try {
    ...
    const sortedSource = sourceEntries.toSorted((left, right) => left.localeCompare(right))
    const sortedTarget = targetEntries.toSorted((left, right) => left.localeCompare(right))
    if (JSON.stringify(sortedSource) !== JSON.stringify(sortedTarget)) return false
