...
export const planCapabilityLink = async (link: CapabilityTarget["links"][number]) => {
  if (!(await pathExists(link.source))) return undefined
  if (!(await pathExists(link.target))) return { target: link.target, operation: "add" as const }
  if (link.method === "copy") {
  ...
  if (!stat.isSymbolicLink()) return { target: link.target, operation: "update" as const }
