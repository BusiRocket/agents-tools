...
export const verifyCapabilityLink = async (link: CapabilityTarget["links"][number]) => {
  if (!(await pathExists(link.source))) {
    return { target: link.target, status: "source-missing", detail: link.source }
  }
  if (!(await pathExists(link.target))) {
    return { target: link.target, status: "target-missing", detail: link.source }
  }
  ...
  if (!(await pathExists(actual))) return { target: link.target, status: "broken", detail: actual }
