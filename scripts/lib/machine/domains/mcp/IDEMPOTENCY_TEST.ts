...
void test("a second apply changes nothing", async () => {
  ...
  const changes = plan({ manifest, state: await read(paths), owned: second.owned, env: TEST_ENV })
...
void test("a foreign key added between runs is not disturbed", async () => {
  ...
  const config = JSON.parse(await readFile(paths["claude-personal"], "utf8")) as {
