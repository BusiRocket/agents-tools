...
void test("a quoted scalar loses its quotes", () => {
  assert.deepEqual(normalizeCodexServer({ command: '"codegraph"' }), { command: "codegraph" })
})

void test("an inline array becomes a string array", () => {
  assert.deepEqual(normalizeCodexServer({ command: '"x"', args: '["serve", "--mcp"]' }), {
    command: "x",
    args: ["serve", "--mcp"],
  })
})
...
void test("a url server normalizes to a url", () => {
  assert.deepEqual(normalizeCodexServer({ url: '"https://mcp.context7.com/mcp"' }), {
...
void test("an escaped quote inside a value is unescaped", () => {
  assert.deepEqual(normalizeCodexServer({ command: '"a\\"b"' }), { command: 'a"b' })
})
