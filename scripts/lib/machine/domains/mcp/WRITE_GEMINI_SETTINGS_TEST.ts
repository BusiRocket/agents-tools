...
void test("Gemini settings remove only stale owned MCP servers", async () => {
  ...
  await writeFile(
    path,
    JSON.stringify({ mcpServers: { stale: { command: "old" }, foreign: { command: "keep" } } }),
  )

  const owned = await writeGeminiSettings({ path, servers: {}, ownedNames: ["stale"] })
...
void test("Gemini settings reject corrupt JSON", async () => {
  ...
  await assert.rejects(writeGeminiSettings({ path, servers: {}, ownedNames: [] }), SyntaxError)
})
