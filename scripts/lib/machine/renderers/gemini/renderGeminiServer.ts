...
export const renderGeminiServer = (server: McpServer, env: NodeJS.ProcessEnv) => {
  ...
  const timeout =
    server.startup_timeout_sec === undefined ? {} : { timeout: server.startup_timeout_sec * 1000 }

  if (server.transport === "stdio") {
    const args = toStringArgs(server.args, server.target_overrides?.gemini?.args_append ?? [])
    return {
      rendered: {
