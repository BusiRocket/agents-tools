...
export const detectPlatformRuntime = async (
  ...
  const commands = await Promise.all(
    (platform.probe.commands ?? []).map(async (candidate): Promise<ProbeResult> => {
      const resolvedPath = await findCommandOnPath(candidate, options.env.PATH)
      return {
        kind: "command",
        candidate,
  ...
  const apps = await Promise.all(
    (platform.probe.appPaths ?? []).map(async (candidate): Promise<ProbeResult> => {
  ...
  const configFound = probes.some(({ found, kind }) => found && kind === "config")
