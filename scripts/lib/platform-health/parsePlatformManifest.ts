...
export const parsePlatformManifest = (raw: unknown): PlatformManifestParseResult => {
  ...
  if (!Array.isArray(manifest.platforms)) {
    return {
      ok: false,
      errors: [...errors, "platforms must be an array"].toSorted((left, right) =>
  ...
  for (const [index, platform] of manifest.platforms.entries()) {
    const registryId = collectPlatformDefinitionErrors(platform, index, registryIds, errors)
    if (registryId === undefined) continue
    if (seen.has(registryId)) errors.push(`${registryId}: duplicate registry id`)
    seen.add(registryId)
  }

  for (const registryId of registryIds) {
    if (!seen.has(registryId)) errors.push(`${registryId}: registered platform is missing`)
  }

  if (errors.length > 0) {
    return { ok: false, errors: errors.toSorted((left, right) => left.localeCompare(right)) }
  }
