...
export function renderBundleWithMdcFrontmatter(
  title: string,
  subtitle: string,
  bundle: { rel: string; frontmatter: Record<string, unknown>; content: string }[],
...
`
  const sections = bundle.map((item) => {
    ...
    const titleLine = typeof frontmatter.description === "string" ? frontmatter.description : rel
    const extra = getExtraFrontmatter(item)
    const extraLines = Object.entries(extra)
      .map(([k, v]) => `${k}: "${v}"`)
      .join("\n")

    const globs = typeof frontmatter.globs === "string" ? frontmatter.globs : "*"
    const alwaysApply =
      typeof frontmatter.alwaysApply === "boolean" ? String(frontmatter.alwaysApply) : "false"
