...
void test("scanned skillkit output is imported into the canonical library", async () => {
  ...
  await writeFile(join(installedSkillsDir, "brainstorming", "SKILL.md"), "new\n")
  await writeFile(join(libraryDir, "skills", "superpowers:brainstorming", "SKILL.md"), "old\n")
  ...
  assert.equal(
    await readFile(join(libraryDir, "skills", "superpowers:brainstorming", "SKILL.md"), "utf8"),
    "new\n",
  )
})
