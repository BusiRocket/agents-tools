...
void test("the lock and affected destinations are snapshotted before installation", async () => {
  ...
  await mkdir(join(libraryDir, "skills", "superpowers:brainstorming"), { recursive: true })
  await writeFile(join(libraryDir, ".skill-lock.json"), "lock-before\n")
  await writeFile(join(libraryDir, "skills", "superpowers:brainstorming", "SKILL.md"), "before\n")
  ...
  assert.equal(
    await readFile(
      join(snapshot.snapshotDir, "skills", "superpowers:brainstorming", "SKILL.md"),
      "utf8",
    ),
    "before\n",
  )
  await assert.rejects(access(join(snapshot.snapshotDir, "skills", "superpowers:writing-plans")))
  assert.match(await readFile(snapshot.manifestPath, "utf8"), /"existed": false/)
})
