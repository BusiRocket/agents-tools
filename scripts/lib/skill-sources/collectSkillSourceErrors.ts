...
export const collectSkillSourceErrors = (
  ...
  if (typeof source.source !== "string" || !/^[^/\s]+\/[^/\s]+$/.test(source.source)) {
    errors.push(`${prefix}.source must be an owner/repository slug`)
  }
  if (typeof source.resolvedCommit !== "string" || !/^[a-f0-9]{40}$/.test(source.resolvedCommit)) {
  ...
  if (!Array.isArray(source.skills) || source.skills.length === 0) {
    ...
    source.skills.some(
      (skill) => typeof skill !== "string" || !/^[a-z0-9]+(?:-[a-z0-9]+)*$/.test(skill),
    )
  ) {
  ...
  } else if (source.targets.some((target) => typeof target !== "string" || target === "")) {
  ...
  if (
    ...
    errors.push(`${prefix}.securityExceptions contains an invalid reviewed exception`)
  }
}
