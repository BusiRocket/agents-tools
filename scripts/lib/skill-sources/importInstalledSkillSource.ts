...
export const importInstalledSkillSource = async (
  ...
  for (const skill of source.skills) {
    ...
    const exists = await access(installedPath)
      .then(() => true)
      .catch(() => false)
    if (!exists) throw new Error(`${skill}: skillkit did not create ${installedPath}`)
    const destinationPath = join(libraryDir, "skills", resolveSkillSourceLogicalName(source, skill))
