...
export const parseSkillSourceManifest = (raw: unknown): SkillSourceManifestParseResult => {
  ...
  const errors: string[] = []
  if (record.version !== 1) errors.push("skill source manifest version must be 1")
  if (!Array.isArray(record.sources)) errors.push("skill source manifest needs a sources array")
