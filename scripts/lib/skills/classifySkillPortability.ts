...
export const classifySkillPortability = (
  ...
  if (/(?:^|\/)(?:fixtures?|examples?|samples?|test-data)(?:\/|$)/i.test(normalizedPath)) {
    return { path, kind: "fixture", fields: [], reasons: ["path is reserved for fixtures"] }
  }
  const { frontmatter } = splitFrontmatter(contents.replaceAll("\r\n", "\n"))
  if (frontmatter === "") {
    return { path, kind: "invalid", fields: [], reasons: ["frontmatter is missing or malformed"] }
  }
  ...
  if (errors.length > 0) return { path, kind: "invalid", fields, reasons: errors }
  ...
  if (targetFields.length > 0 || name.includes(":")) {
    return {
      ...
      fields,
      reasons: name.includes(":") ? ["logical name requires a filesystem alias"] : targetFields,
    }
  }
