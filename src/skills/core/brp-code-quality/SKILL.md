...
- The baseline includes agent-ready documentation, not just tooling. Load
  `references/agent-ready-standard.md` for the full standard and `references/AGENTS.template.md` as
  the starting file; facts go in AGENTS.md only, CLAUDE.md is a one-line `@AGENTS.md` shim, and
  every smoke-test recipe is verified by running it before it is written down.
...
5. Re-run the checks and report what changed, what is left, and any residual risk.

## Output

- Return: findings from the audit, hardening steps applied, check results, agent-ready docs written
  or updated, and the remaining gaps with their risk.
