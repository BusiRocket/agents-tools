...
fixed. Use when the task is to implement a concrete bug fix and the likely cause is already narrow
enough to change code safely while preserving code quality and useful observability signals. Not for
open-ended root-cause investigations, behavior-preserving refactors, or broad code-quality audits.
...
- Load `bug-reproduction-template.md` (same directory) when the bug report is incomplete or
  ambiguous.
